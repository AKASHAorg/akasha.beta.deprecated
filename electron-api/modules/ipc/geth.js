...
class GethService {
    ...
    _startGethService ( event, arg ) {
        this.getGethService().start().then(( data ) => {
          ...
          this._sendEvent(event)(this.clientEvent.startSyncing, true, "Start synchronizing with the network");
    ...
    _getGethUpdates ( event, arg ) {
            this.updatesFlag = setInterval( () => this._getBlockUpdates(event), this.BLOCK_UPDATE_INTERVAL);
    }
